...
```bash
./run.sh gazebo_sim "name_of_world"
```
...
To force the 2D Hokuyo instead of the current 3D lidar path:

```bash
./run.sh slam lidar:=2d
```
...
If you are mapping with the 2D lidar, start RViz the same way:

```bash
./run.sh nav2_rviz lidar:=2d
```
...
### Pause And Resume A SLAM Session

If you do not want to scan the whole world in one sitting, you can save the live
`slam_toolbox` pose graph and continue later.

Save a checkpoint while `./run.sh slam` is running:

```bash
./run.sh slam_save_state baylands
```

This writes:

- `maps/slam_states/baylands/baylands.posegraph`
- `maps/slam_states/baylands/baylands.data`
- `maps/baylands.yaml`
- `maps/baylands.pgm`

Later, after starting Gazebo for the same world again, resume mapping from that
checkpoint:

```bash
./run.sh gazebo_sim baylands false
./run.sh slam_resume baylands
```

By default resume starts from the first node in the saved graph, which is the
right choice when the robot respawns near the same starting pose. You can also
pass extra SLAM launch args through the resume command, for example:

```bash
./run.sh slam_resume baylands sync:=false
```

The resume wrapper now starts `slam_toolbox` directly with the serialized
pose-graph configured as `map_file_name`, which avoids the stale-time restore
problems that can happen when deserializing into an already-running node after a
sim restart. If a heavy 3D lidar world still needs extra time before SLAM
starts, you can increase the built-in warmup:

```bash
./run.sh slam_resume baylands resume_warmup_s:=5.0
```
