...
The first bounded headless Task 5 live run produced detector records with `valid:false` and no dji1 typed hazard because no explicit downward dji1 camera pitch was commanded; it was not a model, projector, fusion, or source-selection failure. A bounded retry with the dji1-only camera scanner fixed at `-60.0` degrees produced non-empty typed hazards at dji1, dji0, and the UGV, selected dji1, preserved source covariance, and preserved dji0-to-UGV forwarding. This validates the live typed contract under the corrected camera orientation only; deterministic synthetic/replayed Task 6/7 checks remain the Task 8 pass basis, and no detector-accuracy, environmental-hazard, closed-loop-navigation, or quantitative-safety claim is made.
